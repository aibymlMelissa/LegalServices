...
esg_goals = "to align corporate culture with strategic management, address value and mission realignment through innovative startup-like approaches"
# Default ESG project types for shaping corporate culture
esg_project_types = [
    ...
    "Environmental innovation incubator",
...
focus_areas = [
    ...
    "Startup-inspired initiative processes",
...
class AgentState(TypedDict):
    ...
    company_status_notes: Optional[str]  # Notes from first client meeting
    selected_esg_project_type: Optional[str]  # Selected ESG project type
...
class ESGProposalDesigner:
    def __init__(self, materials_dir: str = "reference_materials"):
        ...
        self.PLAN_PROMPT = (
            "You are a strategic management consultant specializing in corporate culture transformation. "
            "You're tasked with planning a business proposal that addresses misalignment between a company's "
            "strategic direction and its corporate culture, values, and mission. "
            "Create a structured outline for an initiative that identifies strategic management problems, "
            "diagnoses cultural misalignments, and provides a framework for how corporate culture could be "
            "built to address value and mission realignment. "
            "Begin with an executive summary (two paragraphs) that highlights the strategic importance "
            "of culture-strategy alignment and the benefits of implementing this transformation initiative. "
            "Include sections for: company background, current status assessment, strategic gap analysis, "
            "cultural misalignment diagnosis, innovative management solutions, startup-inspired approach to change, "
            "milestone-based proof of concept framework, progression timeline from prototype to "
            "Minimum Viable Service Product (MVSP), resource requirements, expected outcomes, and measurement criteria. "
            ...
            f"This proposal is designed for {target_industry} organizations seeking to transform corporate culture with "
            "innovative, entrepreneurial approaches to strategic alignment."
        )
        
        self.DRAFT_PROMPT = (
            f"This strategic management proposal should address the following key areas: {', '.join(focus_areas)}. "
            f"Use the selected ESG project type as a starting point to shape the corporate culture: {{selected_esg_project_type}}. "
            "Incorporate the notes from the first client meeting about the current company status: {company_status_notes}. "
            "The proposal must directly identify problems in the client's strategic management "
            "and provide direction on how corporate culture could be built to address "
            "value and mission realignment. "
            "Include innovative management suggestions that challenge traditional approaches and introduce "
            "new ideas on initiative processes in the client's company. "
            "The proposal should demonstrate how a startup-inspired corporate culture transformation can drive strategic value through "
            "enhanced mission alignment, operational efficiency, employee engagement, and strengthened stakeholder relationships. "
            "Design the initiative as a startup within the organization, complete with: "
            "1) Clear milestones for proof of concept with metrics for success "
            "2) A detailed progression from prototype to Minimum Viable Service Product (MVSP) "
            "3) Agile implementation methodologies for rapid iteration and improvement "
            "4) Lean resource allocation models to maximize impact with minimal investment "
            "Address how the organization can implement these innovative cultural change measures, integrate core values into "
            "daily operations using startup methodologies, and strengthen strategic governance structures with "
            "quantifiable metrics and reporting frameworks borrowed from venture development models. "
            "You are a strategic management consultant and innovation strategist tasked with developing a comprehensive business proposal based on the provided plan. "
            "Generate the most compelling and actionable proposal possible, backed by data, best practices, and innovative thinking. "
            "If critique is provided, respond with a revised version addressing the feedback. "
            f"Use a {proposal_style} tone throughout the document while conveying an entrepreneurial spirit."
        )
        
        self.FINALIZE_PROMPT = (
            "You are a senior strategic management advisor, cultural transformation expert, and innovation consultant reviewing this proposal. "
            ...
            "Evaluate the innovative management suggestions for originality, feasibility, and alignment with strategic objectives. "
            "Assess the strength of the new ideas proposed for initiative processes in the client's company. "
            "Review the 'initiative-as-a-startup' framework for coherence and applicability. "
            "Verify that the milestones for proof of concept are clearly defined, measurable, and realistic. "
            "Analyze the progression path from prototype to Minimum Viable Service Product (MVSP) for completeness and practicality. "
            "Verify that implementation steps incorporate agile methodologies and include clear metrics for measuring cultural change. "
            "Check that financial considerations follow lean startup principles, including initial minimal investment and potential ROI. "
            "Ensure that industry-specific cultural challenges and strategic requirements are adequately addressed within the innovative framework. "
            "Consider potential stakeholder concerns about both cultural transformation and strategic realignment, and how they are addressed. "
            "Provide detailed recommendations for improvement, including specific edits, additional innovative elements, "
            "and alignment with current best practices for corporate culture transformation using entrepreneurial approaches. "
            f"The proposal should achieve {esg_goals} effectively while demonstrating entrepreneurial thinking."
        )
    ...
    def plan_node(self, state: AgentState):
        ...
        # Get company status notes and selected ESG project type
        company_status_notes = state.get('company_status_notes', "No status notes provided")
        selected_esg_project_type = state.get('selected_esg_project_type', "No ESG project type selected")
        
        # Construct a context message from the retrieved documents
        context = ""
        if retrieved_docs:
            context = "Here are some relevant reference materials that may help with this strategic management proposal:\n\n"
            for i, doc in enumerate(retrieved_docs):
                context += f"Document {i+1} - Source: {doc['source']}\n"
                context += f"{doc['content']}\n\n"
        
        # Add client meeting notes and ESG project type to the context
        context += f"\nNotes from first client meeting about company status:\n{company_status_notes}\n\n"
        context += f"Selected ESG project type as starting point for culture transformation:\n{selected_esg_project_type}\n\n"
        ...
        return {
            ...
            "count": 1,
            "task": state.get('task', ""),
            "company_status_notes": company_status_notes,
            "selected_esg_project_type": selected_esg_project_type,
    ...
    def draft_node(self, state: AgentState):
        try:
            ...
            # Format draft prompt with the company status notes and selected ESG project type
            formatted_draft_prompt = self.DRAFT_PROMPT.format(
                company_status_notes=company_status_notes,
                selected_esg_project_type=selected_esg_project_type
            )
            
            user_message = HumanMessage(
                content=f"{task}\n\nHere is my strategic management proposal plan:\n\n{plan}")
            
            messages = [
                SystemMessage(content=formatted_draft_prompt),
                user_message
            ]
