...
class ESGProposalGUI:
    ...
    def create_sample_params(self):
        ...
        if not list(self.params_dir.glob("*.json")):
            sample_params = {
                "proposal_title": "Strategic Culture Alignment for Growth-Focused Enterprises",
                "target_company": "Medium to Large Enterprise",
                "industry": "Technology",
                "company_status_notes": "Company experiencing high turnover and misalignment between stated values and operational practices. Recent strategic pivot not well integrated into daily operations. Leadership seeking to rebuild culture around innovation.",
    ...
    def save_new_parameters(title, company, industry_type, company_status, esg_project, areas):
        try:
            ...
            params = {
                ...
                "company_status_notes": company_status,
                "selected_esg_project_type": esg_project,
    ...
    def load_parameters(self, file_name):
        ...
        try:
            ...
            display_text += f"Company Status Notes: {params.get('company_status_notes', 'N/A')}\n\n"
            display_text += f"Selected ESG Project Type: {params.get('selected_esg_project_type', 'N/A')}\n\n"
    ...
    def run_agent(self, start, topic, stop_after):
        def get_company_status_notes(self):
            """Get the current company status notes from the state."""
            try:
                current_values = self.graph.get_state(self.thread)
                if current_values and hasattr(current_values, 'values'):
                    return current_values.values.get('company_status_notes', "No status notes provided")
                return "No status notes provided"
            except Exception:
                return "No status notes provided"

        def get_selected_esg_project_type(self):
            """Get the selected ESG project type from the state."""
            try:
                current_values = self.graph.get_state(self.thread)
                if current_values and hasattr(current_values, 'values'):
                    return current_values.values.get('selected_esg_project_type', "No ESG project type selected")
                return "No ESG project type selected"
            except Exception:
                return "No ESG project type selected"
        try:
    ...
    def create_interface(self):
        with gr.Blocks(theme=gr.themes.Default(spacing_size='sm', text_size="sm")) as demo:
            ...
            with gr.Tab("Proposal Parameters"):
                gr.Markdown("""
                ## Strategic Management & Corporate Culture Parameters

                **Getting Started:**
                1. Define your proposal parameters below based on your first client meeting
                2. Be sure to include notes about the current company status and select an ESG project type
                3. After setting parameters, proceed to the Reference Materials tab to add supporting documents
                4. Then go to the Agent tab to generate your strategic management proposal

                The parameters below will help tailor the proposal to address specific corporate culture and strategic alignment needs.
                """)
                ...
                with gr.Accordion("Define New Parameters", open=True):
                    with gr.Group():
                        proposal_title = gr.Textbox(
                            ...
                            placeholder="e.g., Strategic Culture Alignment for Growth-Focused Enterprises",
                            info="The main title of your strategic proposal"
                        )
                        
                        target_company = gr.Textbox(
                            label="Target Company",
                            placeholder="e.g., Mid-sized technology company, Global enterprise",
                        ...
                        # Get the ESG project types from the defaults in project_designer.py
                        esg_project_types = [
                            "Carbon footprint reduction initiative",
                            "Sustainable supply chain transformation",
                            "Social responsibility program",
                        ...
                        company_status_notes = gr.Textbox(
                            label="Notes about current company status",
                            placeholder="e.g., High turnover, misalignment between stated values and operations, recent strategic pivot",
                            info="Key insights from the first client meeting about current status",
                            lines=4
                        )
                        
                        esg_project_type = gr.Dropdown(
                            label="ESG project type for shaping corporate culture",
                            choices=esg_project_types,
                            info="Select the ESG project type to use as a starting point for culture transformation",
                        )
                        
                        focus_areas = gr.Textbox(
                            label="Additional Focus Areas (Optional)",
                            placeholder="e.g., Corporate culture transformation, Value and mission realignment",
                            info="Comma-separated list of additional focus areas",
                            lines=3
                        )
                ...
                save_params_btn.click(
                    fn=self.save_new_parameters,
                    inputs=[proposal_title, target_company, industry, company_status_notes, esg_project_type, focus_areas],
                    outputs=[save_result, param_files]
                )

                # Apply parameters to current proposal
                apply_params_btn.click(
                    fn=lambda title: (title, "Parameters applied to proposal. Go to the Agent tab to generate content.") if title else (gr.update(), "No proposal title provided. Please define parameters first."),
                    inputs=[proposal_title],
                    outputs=[topic_bx, save_result]
                )
                
            
            with gr.Tab("Reference Upload"):
                gr.Markdown("""
